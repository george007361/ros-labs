...
enum ControlEnum
{
    ...
    WALLFOLLOWER_V2, // раскомментировать после добавления нового алгоритма
    nControls
};
...
int main(int argc, char **argv)
{
    ...
    controls[WALLFOLLOWER] = new WallFollower(
        node.param("line_y", -10.0),
        node.param("cx", 6),
        node.param("cy", 0),
        node.param("R", 6),
        node.param("task_vel", 1.0),
        node.param("prop_factor", 0.1),
        node.param("int_factor", 0.0),
        node.param("diff_factor", 0.0),
        node.param("min_obstacle_range", 1.0));

    controls[WALLFOLLOWER_V2] = new WallFollowerNew();
